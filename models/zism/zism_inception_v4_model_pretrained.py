...
class InceptionV4(nn.Module):
    def __init__(self, in_channels=3, num_classes=6, k=192, l=224, m=256, n=384):
        ...
        blocks1 = []
        blocks1.append(Stem(1))
        for i in range(4):
            blocks1.append(Inception_A(384))
        blocks1.append(Reduction_A(384, k, l, m, n))
        for i in range(7):
            blocks1.append(Inception_B(1024))
        blocks1.append(Reduction_B(1024))
        for i in range(3):
            blocks1.append(Inception_C(1536))
        self.features1 = nn.Sequential(*blocks1)
        # self.global_average_pooling = nn.AdaptiveAvgPool2d((1, 1))
        # self.linear = nn.Linear(1536, classes)
        self.conv1_fusion = nn.Conv2d(4608, 256, kernel_size=3, stride=1, padding=1, bias=False)
        ...
        self.downsample = nn.Sequential(
            nn.Conv2d(4608, 128, kernel_size=1, stride=1, padding=0, bias=False),
            nn.BatchNorm2d(128),
        )
...
#     input1 = torch.randn(16, 1, 128, 128)
#     input2 = torch.randn(16, 1, 128, 128)
#     out = model(input, input1, input2)
#     print(out.shape)
